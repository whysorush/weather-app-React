...
export default function Weather() {
  ...
  const [weather, setWeather] = React.useState("mumbai");
  const [dataJson, setDataJson] = React.useState(null);

  const clickHandler = (event) => {
    console.log("clickHandler")
    setWeather(city)
    console.log(dataJson.temp)
  }
  ...
  function weatherJson (props){
    // for()
    return <li> hello {props.temp}</li>
  }
  const fetchData = async () => {

    const response = await fetch(process.env.REACT_APP_API + weather, {
      method: 'GET',
      headers: {
  ...
  React.useEffect(() => {
    console.log("useEffect", process.env.REACT_APP_API)
    fetchData()
  }, [weather]);
  ...
  return (
    <>
      <div class="container">
      <h1 class="app-heading">Enter Your city name :</h1>
        {/* <label></label> */}
        {/* {dataJson} */}
      </div>
      ...
      <div class="container">
        <div>
        <button class="button button-block" onClick={clickHandler}>search</button>
        </div>
      </div>
      <div class="container">
          ...
          <div class="app-content">
            ...
            <div class="button button-dial">
              ...
              <div class="button-dial-label">
                ...
                {dataJson ? dataJson.temp : 7}&deg;C
              </div>
            </div>
            <ul>
              
                {/* {Object.values(dataJson).map((data) => <weatherJson dataSource={data} />)} */}
                <li> {dataJson ? Object.keys(dataJson)[0] +" = "+ Object.values(dataJson)[0]  : 7}</li>
                <li> {dataJson ? Object.keys(dataJson)[1] +" = "+ Object.values(dataJson)[1]  : 7}</li>
                <li> {dataJson ? Object.keys(dataJson)[2] +" = "+ Object.values(dataJson)[2]  : 7}</li>
                <li> {dataJson ? Object.keys(dataJson)[3] +" = "+ Object.values(dataJson)[3]  : 7}</li>
                <li> {dataJson ? Object.keys(dataJson)[4] +" = "+ Object.values(dataJson)[4]  : 7}</li>
                <li> {dataJson ? Object.keys(dataJson)[5] +" = "+ Object.values(dataJson)[5]  : 7}</li>
                <li> {dataJson ? Object.keys(dataJson)[6] +" = "+ Object.values(dataJson)[6]  : 7}</li>
            </ul>
